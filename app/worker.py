...
def scrape_e_alertar(termos: list[str]) -> dict:
    """Executa scraping e envia alertas para ofertas com score >= 75."""
    ...
    async def _executar() -> dict:
        ...
        for termo in termos:
            try:
                ofertas = await scraper.buscar(termo)
            except Exception:
                continue
            for oferta in ofertas:
                ...
                if resultado.deve_alertar:
                    await enviar_alerta(oferta, resultado.score)
                    alertas_enviados += 1
        return {"termos": termos, "alertas_enviados": alertas_enviados}

    # asyncio.run() chamado UMA VEZ: cria e destrói um único event loop por task
